...
impl Garfanzo {
    ...
    fn subcommand_window_new<'a, 'b>() -> App<'a, 'b> {
        SubCommand::with_name("new")
            ...
            .arg(Arg::with_name("content").help("What kind of content the window will contain (T(ext) | L(ist) | SB (ScoreBoard) | PB (ProgressBar))").possible_values(&["text", "list", "scoreboard", "bar"])
                .index(2).takes_value(true))
            // TODO fill in message help
            .arg(Arg::with_name("message").help("The message or data to be displayed inside the window.")
                .index(3).takes_value(true))
            // TODO fill in style help
            .arg(Arg::with_name("style").help("How the contents will get styled. (If styling Text, List, or ScoreBoard [bold | highlight | blink | underline]) | (If styling a ProgressBar [[low | blink], <character>]")
